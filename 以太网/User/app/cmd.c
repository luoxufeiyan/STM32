...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define IN_USER 	"Log in as:\r\n"  //输入用户名提示
#define USER_ERR 	"UserName wrong:\r\n"			   //用户名错误提示
#define IN_PWD 		"Password:\r\n"  //输入密码提示
#define PWD_ERR 	"\r\nPassword wrong:\r\n"
#define IN_CMD 		"\r\nPlease enter command:\r\n"	  //输入命令提示
#define CMD_ERR		"\r\nCommand not found!"
#define STATUS_ON	"\r\n LED1 status is ON!Excited~\r\n"
#define STATUS_OFF	"\r\n LED1 status is OFF!Excited~\r\n"
...
//        name->bytes[name->length-1] = '\n';	 
		...
		dat->bytes[dat->length-2] = '\0'; //加结束符，提取出bytes的数据
	   	...
	   	if(strcmp(r_msg.user,"lxfy")==0) //已输入过用户名
			{
				//	 printf("\r\n r_msg.user=%s",r_msg.user);

			   if(strcmp(r_msg.PWD,"12356789")==0)	//已输入过密码
			   		{
			...
			{
				if(strcmp(dat->bytes,"lxfy")==0)	//用户名正确
				{
