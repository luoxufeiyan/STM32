    ...
    if(count ==time)		 					
    {
        ...
        for(i=1; i<time; i++)
        {
            ...
            if(buffer[1][i]>max_y)
            {
				...
				/*去除最小值和最大值之后求平均值*/
        screen2.x=(buffer[0][0]+buffer[0][1]+buffer[0][2]-min_x-max_x)>>3;
        screen2.y=(buffer[1][0]+buffer[1][1]+buffer[1][2]-min_y-max_y)>>3; 
